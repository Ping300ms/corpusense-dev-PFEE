...
const TestPage = () => {
  ...
  const handleSyncPending = async () => {
    await sync.pushPendingOperations();
    await loadData();
  };
  ...
  const handleDeleteTable = async <T extends Syncable>(type: keyof typeof db) => {
    ...
    for (const entity of entities) await sync.delete(entity.id, type);
    await table.clear();
    await loadData();
  };
    ...
    <div className="panel h-full w-full flex flex-col space-y-4">
      ...
      <section className="panel space-y-2">
        ...
        <div className="flex space-x-2">
          ...
          <Button variant="secondary" onClick={() => void handleSyncPending()}>Sync Pending</Button>
        ...
        <ul className="list-disc pl-5 space-y-1 text-sm">
          {collections.map((c) => (
            <li key={c.id} className="flex justify-between items-center border-b pb-1">
              <div>
                {c.name} <span className="text-muted-foreground">– {c.updated_at}</span>
              </div>
        ...
        <ul className="list-disc pl-5 space-y-1 text-sm">
          {annotations.map((a) => (
            <li key={a.id} className="flex justify-between items-center border-b pb-1">
              <div>
                {a.id} – order: {a.order} <span className="text-muted-foreground"></span>
              </div>
        ...
        <ul className="list-disc pl-5 space-y-1 text-sm">
          {models.map((m) => (
            <li key={m.id} className="flex justify-between items-center border-b pb-1">
              <div>
                {m.name} <span className="text-muted-foreground"> – {m.updated_at}</span>
              </div>
